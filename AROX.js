...
const sayiEmojiler = {
  0: "<a:sayi0:784424877763461180>",
  1: "<a:sayi1:784424885694758912>",
  2: "<a:sayi2:784424886403334154>",
  3: "<a:sayi3:784424883270320170>",
  4: "<a:sayi4:784424881302405160>",
  5: "<a:sayi5:784424880070066176>",
  6: "<a:sayi6:784424884913700864>",
  7: "<a:sayi7:784424883807453205>",
  8: "<a:sayi8:784424882049253387>",
  9: "<a:sayi9:784424879217836032>"
};
...
client.emojiler = {
  onay: "784424860411494411",
  iptal: "784424872595947570",
  cevrimici: "784424889074712586",
  rahatsizetmeyin: "784424873283420210",
  bosta: "784424872063139920",
  gorunmez: "784424888386715648",
  booster: "784424973832814632",
  erkekEmoji: "719668058129432607",
  kizEmoji: "719668167936311307",
  gif1: "740178616389730307",
  gif2: "771421979445035008",
  gif3: "771421964652249088",
  gif4: "740178603744034858"
};
...
client.on("messageUpdate", async (oldMessage, newMessage) => {
  ...
  let aroxLog = newMessage.guild.channels.cache.find(c => c.name === "endless-mesaj-log");
  if (oldMessage.content == newMessage.content) return;
  let aroxDelete = new MessageEmbed()
    .setColor("RANDOM")
    .setAuthor(`Mesaj Düzenlendi`, newMessage.author.avatarURL({dynamic: true}))
    ...
    .setFooter(
      `Arox Log's | Bugun Saat ${newMessage.createdAt.getHours() +
        3}:${newMessage.createdAt.getMinutes()}`,
      `${client.user.displayAvatarURL()}`
    );
  aroxLog.send(aroxDelete);
});
//
client.on("messageDelete", async deletedMessage => {
  if (deletedMessage.author.bot || deletedMessage.channel.type === "dm") return;
  let aroxLogs = deletedMessage.guild.channels.cache.find(
    x => x.name === "endless-mesaj-log"
  );
  let aroxDeleted = new MessageEmbed()
    .setColor("RANDOM")
    .setAuthor(`Mesaj Silindi`, deletedMessage.author.avatarURL({dynamic: true}))
    ...
    .setFooter(
      `Arox Log's | Bugün Saat ${deletedMessage.createdAt.getHours() +
        3}:${deletedMessage.createdAt.getMinutes()}`,
      `${client.user.displayAvatarURL()}`
    );
  aroxLogs.send(aroxDeleted);
});
